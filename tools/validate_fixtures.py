...
claim coverage of geometry-dependent SCHEMA/XACC cases.
"""
...
ID_FIELDS = {
    "orthosisProjects": "orthosisProjectId", "assets": "assetId", "frameDefinitions": "frameId",
    "acquisitions": "acquisitionId", "registrations": "registrationId", "landmarkSets": "landmarkSetId",
    "roiDefinitions": "roiId", "designRevisions": "revisionId", "outcomeTargets": "targetId",
    "outcomeMeasurements": "measurementId", "outcomeComparisons": "comparisonId", "promMeasurements": "measurementId",
    "materialLots": "lotId", "mechanicalPropertyMeasurements": "measurementId", "manufacturingRuns": "runId",
    "manufacturingArtifacts": "artifactId", "physicalOrthoses": "physicalPartId", "qcRequirements": "requirementId",
    "qcMeasurements": "measurementId", "provenanceRecords": "provenanceId", "migrationHistory": "migrationId",
}
...
def base_checks(p: dict) -> list[str]:
    ...
    for n in parents: visit(n)
    passed.append("SCHEMA-005")

    defs = {}
    for bucket, typ in DEF_TYPES.items():
        for d in p["definitions"].get(bucket, []): defs[(typ, d["id"], d["version"])] = d
    ...
    if p["registrations"]: passed.append("SCHEMA-011")
    ...
    for m in p["outcomeMeasurements"]:
        ...
        if m.get("roiRef"): need(m["roiRef"] in rois, "SCHEMA-012", "outcome ROI missing")
    if p["outcomeMeasurements"]: passed += ["SCHEMA-012", "SCHEMA-014"]
    ...
    if p["promMeasurements"]: passed.append("SCHEMA-016")
    ...
    if p["mechanicalPropertyMeasurements"]: passed.append("SCHEMA-017")

    runs = {x["runId"]: x for x in p["manufacturingRuns"]}; arts = {x["artifactId"]: x for x in p["manufacturingArtifacts"]}
    ...
    if arts or p["physicalOrthoses"]: passed.append("SCHEMA-018")

    reqs = {r["requirementId"]: r for r in p["qcRequirements"]}
    for m in p["qcMeasurements"]:
        req = reqs.get(m["requirementId"]); need(req is not None, "SCHEMA-020", "QC requirement missing")
    ...
    if p["qcMeasurements"]: passed.append("SCHEMA-020")

    for m in p["migrationHistory"]:
        need(m.get("informationLoss"), "SCHEMA-023", "migration loss state missing")
    if p["migrationHistory"]: passed.append("SCHEMA-023")

    for pr in p["provenanceRecords"]:
        for out in pr["outputEntityRefs"]: need(out in ids, "SCHEMA-027", f"dangling provenance output {out}")
    if p["provenanceRecords"]: passed.append("SCHEMA-027")
    ...
    if p["case"]["attachedProfiles"]: passed.append("SCHEMA-029")
    return sorted(set(passed))

def mv(m, p):
    v = [*p, 1.0]; o = [sum(float(m[r][c])*v[c] for c in range(4)) for r in range(4)]
...
def acceptance(p: dict) -> list[str]:
    ...
    if fid == "registration-known-transform":
        ...
        for x,y in zip(a["inputPoints"],a["expectedTargetPoints"]): need(dist(mv(m,x),y)<=a["numericToleranceMm"],"XACC-008","known transform mismatch")
        out=["XACC-008","SCHEMA-011"]
    elif fid == "mirror-semantics":
        e=a["expected"]; rs={r["revisionId"]:r for r in p["designRevisions"]}; s,d=rs[e["sourceRevisionId"]],rs[e["targetRevisionId"]]
        need(s["side"]=="RIGHT" and d["side"]=="LEFT","XACC-003","mirror side wrong")
        for k in ("s","q"): need(s["operationStack"][0]["parameters"][k]==d["operationStack"][0]["parameters"][k],"XACC-003",f"{k} changed")
        need(h(s["contentHash"])!=h(d["contentHash"]),"XACC-004","mirror reused source revision"); need(e["roundTripSemanticExactness"],"XACC-005","roundtrip expectation absent")
        out=["XACC-003","XACC-004","XACC-005"]
    elif fid == "roi-version-comparison":
        e=a["expected"]; rois={r["roiId"]:r for r in p["roiDefinitions"]}; need(e["roiV1Id"] in rois and e["roiV2Id"] in rois,"XACC-010","ROI history missing")
        c=next(x for x in p["outcomeComparisons"] if x["comparisonId"]==e["comparisonId"]); need(c["compatibilityState"]=="NOT_COMPARABLE","XACC-010","version mismatch compared silently"); out=["XACC-010","SCHEMA-012"]
    elif fid == "profile-non-transfer":
        e=a["expected"]; need(e["decision"]=="BLOCK","XACC-024","cross-profile target not blocked"); states={x["profileRef"]["id"]:x["confirmationState"] for x in p["case"]["attachedProfiles"]}; need("SUGGESTED_NOT_CONFIRMED" in states.values(),"XACC-023","fixture lacks unconfirmed profile"); out=["XACC-023","XACC-024","SCHEMA-029"]
    elif fid == "prom-versioning":
        x,y=p["promMeasurements"]; need((x["instrumentRef"]["id"],x["instrumentRef"]["version"])!=(y["instrumentRef"]["id"],y["instrumentRef"]["version"]),"XACC-025","PROM history collapsed"); out=["XACC-025","SCHEMA-016"]
    elif fid == "offload-safety-ring":
        e=a["expected"]; required={e["targetRoiId"],e["safetyRingRoiId"],*e["remoteRegionRoiIds"]}; got={r["roiId"] for r in p["roiDefinitions"]}; need(required<=got,"XACC-027","target/safety/remote ROI incomplete"); out=["XACC-027"]
    elif fid == "material-property-provenance":
        e=a["expected"]; ms={m["measurementId"]:m for m in p["mechanicalPropertyMeasurements"]}; n,v=ms[e["nominalMeasurementId"]],ms[e["effectiveMeasurementId"]]; need(n["sourceType"]=="SUPPLIER_NOMINAL" and v["sourceType"]!="SUPPLIER_NOMINAL","XACC-031","nominal/effective provenance collapsed"); out=["XACC-031","SCHEMA-017"]
    elif fid == "blocking-qc":
        e=a["expected"]; part=next(x for x in p["physicalOrthoses"] if x["physicalPartId"]==e["physicalPartId"]); need("acceptedAt" not in part and part["lifecycleState"]!="ACCEPTED","XACC-034","blocking QC leak"); out=["XACC-034","SCHEMA-020"]
    else: raise Failure(f"HARNESS: unknown fixture {fid}")
    missing=set(a.get("testIds",[]))-set(out); need(not missing,"HARNESS",f"declared IDs not executed: {sorted(missing)}")
    return out

def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument("paths",nargs="*"); ns=ap.parse_args()
    schema=json.loads(SCHEMA.read_text(encoding="utf-8")); val=Draft202012Validator(schema,format_checker=FormatChecker())
    files=[ROOT/p for p in ns.paths] if ns.paths else sorted({p for g in GLOBS for p in ROOT.glob(g)})
    failed=0; aggregate=set()
    for path in files:
        p=json.loads(path.read_text(encoding="utf-8")); errs=sorted(val.iter_errors(p),key=lambda e:list(e.absolute_path))
        if errs:
            failed+=1; print(f"FAIL {path.relative_to(ROOT)}"); [print(f"  JSON-SCHEMA: {e.message}") for e in errs]; continue
        try:
            passed=["SCHEMA-001","SCHEMA-008","SCHEMA-030"]+base_checks(p)+acceptance(p); aggregate.update(passed)
            print(f"PASS {path.relative_to(ROOT)} [{', '.join(sorted(set(passed)))}]")
        except Failure as exc:
            failed+=1; print(f"FAIL {path.relative_to(ROOT)}\n  {exc}")
